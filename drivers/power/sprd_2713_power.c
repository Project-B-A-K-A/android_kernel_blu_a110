...
// #define DEBUG_FOR_NTC_TEMP  only for test
static int limit_charging = 0; //1:LIMIT_CHARGING;0:STATUS_CHARGING
#ifdef VBAT_4100_TEMP_45
static int vbat_temp_ctrl = 0;      //0:charging;1:discharging
#endif
...
#if defined(DEBUG_FOR_NTC_TEMP)
int get_temp_for_temp();
#endif
static void sprdbat_info_init(struct sprdbat_drivier_data *data)
{
	...
	#if defined(DEBUG_FOR_NTC_TEMP)
	data->bat_info.cur_temp = get_temp_for_temp();
	#else
...
static int sprdbat_start_charge(void)
{
	...
	if(limit_charging)
		sprdchg_set_chg_cur(sprdbat_data->pdata->otp_low_limit_current);
	else
	...
	if (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT) {
		...
		if (flag_msk != SPRDBAT_CHG_END_FULL_BIT
			...
			sprdbat_data->bat_info.module_state =
			    POWER_SUPPLY_STATUS_CHARGING;
		 #if defined(DEBUG_FOR_NTC_TEMP)
		  SPRDBAT_DEBUG("jinxing _sprdbat_clear_stopflags POWER_SUPPLY_STATUS_CHARGING\n");
		 #endif
		}
...
static void sprdbat_update_capacty(void)
{
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		...
		if (100 == sprdbat_data->bat_info.capacity) {
			...
			if (fgu_capacity >= 100) {
				fgu_capacity = 99;
				SPRDBAT_DEBUG ("soc 100,update charge full\n"); 
...
{
	static num=0;
	int temp_test[44]={660,660,650,650,600,600,580,580,540,540,440,340,240,200,120,120,100,100,10,10,0,0,-10,-10,-10,-10,0,0,10,10,100,100,120,120,540,540,580,580,600,600,650,650,660,660};
	num++;
	if(num>44)
		num=0;

	return temp_test[num%44];
}
#endif
static uint32_t temp_trigger_cnt;
#ifdef VBAT_4100_TEMP_45
static uint32_t vbat_temp_ctrl_cnt = 0;
#endif
static void sprdbat_temp_monitor(void)
{
#if defined(DEBUG_FOR_NTC_TEMP)
sprdbat_data->bat_info.cur_temp = get_temp_for_temp();
SPRDBAT_DEBUG("sprdbat_temp_monitor sprdbat_data->bat_info.cur_temp =%d", sprdbat_data->bat_info.cur_temp);
#else
	sprdbat_data->bat_info.cur_temp = sprdbat_read_temp();
#endif
	...
	if (sprdbat_data->bat_info.
		...
		if (sprdbat_data->bat_info.cur_temp >
			...
			{
				limit_charging =1;
				SPRDBAT_DEBUG("jinxing sprdbat_temp_monitor charge stop-->low_LIMIT CHARGING\n");
		...
		if (sprdbat_data->bat_info.cur_temp <
            ...
            {
			#ifdef VBAT_4100_TEMP_45
			if(sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->otp_high_limit_charge_temp && sprdbat_read_vbat_vol() > 4100){
				vbat_temp_ctrl = 1;
			}else{
				vbat_temp_ctrl = 0;
			}
		...
		if (sprdbat_data->bat_info.cur_temp <
			...
			if (temp_trigger_cnt > SPRDBAT_TEMP_TRIGGER_TIMES) {
				if (sprdbat_data->bat_info.cur_temp <
				    sprdbat_data->pdata->otp_low_stop) {
						limit_charging =0;
						SPRDBAT_DEBUG("jinxing sprdbat_temp_monitor lowlimit-->charge stop!!!\n");
					sprdbat_change_module_state
					    (SPRDBAT_OTP_COLD_STOP_E);
				} else {
				    limit_charging =0;
					...
					vbat_temp_ctrl = 0;
					#endif
					SPRDBAT_DEBUG("jinxing sprdbat_temp_monitor highlimit-->charge stop!!!\n");
					sprdbat_change_module_state
					    (SPRDBAT_OTP_OVERHEAT_STOP_E);
				}
			...
			if(sprdbat_data->bat_info.cur_temp < sprdbat_data->pdata->otp_low_limit_charge_temp 
			    || sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->otp_high_limit_charge_temp){
				if(!limit_charging){
					limit_charging =1;
					sprdchg_set_chg_cur((sprdbat_data->pdata->otp_low_limit_current));
					SPRDBAT_DEBUG("jinxing sprdbat_temp_monitor LIMIT_CHARGING\n");
				}
			}else {
				if(limit_charging){
					limit_charging =0;
					sprdchg_set_chg_cur(sprdbat_data->bat_info.chg_current_type);
					SPRDBAT_DEBUG("jinxing sprdbat_temp_monitor STATUS_CHARGING\n");
				}
			}
			...
			if(sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->otp_high_limit_charge_temp && sprdbat_read_vbat_vol() > 4100){
				vbat_temp_ctrl_cnt++;
				if(vbat_temp_ctrl_cnt > SPRDBAT_TEMP_TRIGGER_TIMES){
					vbat_temp_ctrl_cnt = 0;
					if(!vbat_temp_ctrl){
						vbat_temp_ctrl = 1;
						sprdbat_data->stop_charge();
					}
...
static void sprdbat_charge_works(struct work_struct *work)
{
	...
	if (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT) {
#ifdef CHG_CUR_ADJUST
		if((!thermal_ctrl_chg_cur) && (!limit_charging))
	...
	if (sprdbat_data->bat_info.chg_stop_flags & SPRDBAT_CHG_END_FULL_BIT) {
		     SPRDBAT_DEBUG("fgu_recaps=:%d",fgu_recaps);
		//if (sprdbat_data->bat_info.vbat_ocv <
		//    sprdbat_data->pdata->rechg_vol) {
		if(fgu_recaps<=96)
		{	
...
#define PDATA_LOG(format, arg...) printk("sprdbat pdata: " format, ## arg)
	...
	PDATA_LOG("otp_low_limit_charge_temp:%d\n", pdata->otp_low_limit_charge_temp);
	PDATA_LOG("otp_high_limit_charge_temp:%d\n", pdata->otp_high_limit_charge_temp);
	PDATA_LOG("otp_low_limit_current:%d\n", pdata->otp_low_limit_current);
							   ...
							   *pdev)
{
	...
	ret = of_property_read_u32(np, "otp-low-limit-charge-temp", (u32 *) (&temp));
	pdata->otp_low_limit_charge_temp = temp - 1000;
	ret = of_property_read_u32(np, "otp-high-limit-charge-temp", (u32 *) (&temp));
	pdata->otp_high_limit_charge_temp = temp - 1000;
	ret = of_property_read_u32(np, "otp-low-limit-current", (u32 *) (&temp));
	pdata->otp_low_limit_current = temp;
