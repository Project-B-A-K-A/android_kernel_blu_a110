...
LOCAL int _sensor_k_avdd_gpio(struct sensor_file_tag *fd_handle, uint32_t avdd_val)
{
             struct sensor_module_tab_tag    *p_mod;
             uint32_t level,avdd_gpio;

	SENSOR_CHECK_ZERO(fd_handle);
	p_mod = fd_handle->module_data;
	SENSOR_CHECK_ZERO(p_mod);
             printk("_sensor_k_avdd_gpio avdd_val=%d\n",avdd_val);
             if (0 == avdd_val)
                level = 0;
             else
                level = 1;
	get_gpio_id_ex(p_mod->of_node, GPIO_MAINAVDD, &avdd_gpio, fd_handle->sensor_id);
	SENSOR_PRINT_HIGH("SENSOR: avdd val %d id %d avdd_gpio %d\n",level, fd_handle->sensor_id, avdd_gpio);

	gpio_direction_output(avdd_gpio, level);
	gpio_set_value(avdd_gpio, level);
	//SLEEP_MS(width);
	//SLEEP_MS(50);
	//gpio_set_value(avdd_gpio, !level);
...
LOCAL int _sensor_k_get_camsub_id(struct sensor_file_tag *fd_handle, int *level)
{
	int                              gpio_id = 0;
	int                              ret = SENSOR_K_FAIL;
	struct sensor_module_tab_tag     *p_mod;
	...
	get_gpio_id_ex(p_mod->of_node, GPIO_CAMSUB_ID, &gpio_id, fd->sensor_id);
	if (0 != gpio_id) {
		gpio_direction_input(gpio_id);
		*level  = gpio_get_value(gpio_id);
		SENSOR_PRINT_HIGH("SENSOR: gpio %d, camsub_id %d \n", gpio_id, *level);
		ret = SENSOR_K_SUCCESS;
	} else {
		SENSOR_PRINT_ERR("SENSOR: _sensor_k_get_camsub_id fail \n");
	}
				...
				unsigned long arg)
{
	...
	case SENSOR_IO_SET_AVDD:
		{
			...
			ret = copy_from_user(&vdd_val, (uint32_t *) arg, sizeof(uint32_t));
            #if defined(CONFIG_PROJS_V2804) || defined(CONFIG_PROJS_V2806) || defined(CONFIG_PROJS_V2808)
                    		if (0 == ret)
                                                    ret = _sensor_k_avdd_gpio((uint32_t *)p_file, vdd_val);
                                   #else
                                        if (0 == ret)
	...
	case SENSOR_IO_IF_CFG:
		...
		case SENSOR_IO_GET_CAMSUB_ID:
		{
			uint32_t camsub_id_val;
			SENSOR_PRINT("SENSOR: ioctl SENSOR_IO_GET_CAMSUB_ID \n");
			ret = _sensor_k_get_camsub_id((uint32_t *)p_file, &camsub_id_val);
			if (0 == ret)
				ret = copy_to_user((uint32_t *)arg, &camsub_id_val, sizeof(uint32_t));
		}
		break;
...
int sensor_k_probe(struct platform_device *pdev)
{
	...
	gpio_set_value(gpio_id, 0);
//--------------
             get_gpio_id_ex(p_mod->of_node, GPIO_MAINAVDD, &gpio_id, 0);
	ret = gpio_request(gpio_id, NULL);
	if (ret) {
		tmp = 1;
		printk("sensor: gpio already request GPIO_MAINAVDD,  gpio_id = %d.\n", gpio_id);
	 }
	gpio_direction_output(gpio_id, 1);
	gpio_set_value(gpio_id, 0);

	get_gpio_id_ex(p_mod->of_node, GPIO_SUBAVDD, &gpio_id, 0);
	ret = gpio_request(gpio_id, NULL);
	if (ret) {
		tmp = 1;
		printk("sensor: gpio already request GPIO_SUBAVDD,  gpio_id = %d.\n", gpio_id);
	 }
	gpio_direction_output(gpio_id, 1);
	gpio_set_value(gpio_id, 0);
//--------------
