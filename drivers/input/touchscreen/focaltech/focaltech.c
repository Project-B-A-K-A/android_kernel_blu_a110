...
#if defined(CONFIG_NO_ALSPS_TP_PS)
#define TPD_PROXIMITY
#endif
...
#endif
#define FOCALTECH_PLS_DEVICE_NAME		"epl2182_pls"
#define FOCALTECH_PLS_INPUT_NAME			"proximity"
...
static int factory_start(char *str)
{
        //if (strncmp(str, "normalmode", 10) == 0)
              //  power_mode = 1;
              
		pr_info("[FST] %s: factory_start %s\n",__func__,str);
		if (strncmp(str, "normalmode", 10) == 0)
			power_mode = 1;

        return 1;
}
__setup("androidboot.mode=", factory_start);
...
static int ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[geroge] read chip id error %x\n", uc_reg_value);
	if (err < 0)
	{
...
if (ft_rw_iic_drv_init(client) < 0)	
{
	...
	printk("********************Enter CTP Auto Upgrade******************** %d \n",power_mode);
	if(power_mode)
