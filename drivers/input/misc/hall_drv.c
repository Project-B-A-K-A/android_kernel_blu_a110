                         ...
                         struct device *dev)
{
	...
	printk(" %s:get pdata->gpio_detect  %d\n",__func__,pdata->gpio_detect);
	if(pdata->gpio_detect < 0){
		dev_err(dev, "fail to get irq_gpio_number\n");
		printk(" get irq1_gpio_number fail\n");
		goto fail;
	}
...
static ssize_t hall_chipinfo_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	//return sprintf(buf,"hall_name = %s\n", "hall");
...
static DRIVER_ATTR(chipinfo,          S_IWUSR | S_IRUGO, hall_chipinfo_show,NULL);
