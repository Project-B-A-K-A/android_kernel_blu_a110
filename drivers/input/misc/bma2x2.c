...
#define	CAL_PATH	"/productinfo/BMA2X2_ACC_CAL.bin"
#define CAL_COUNT	8
...
static ssize_t bma2x2_chipinfo_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
short sum_x, sum_y, sum_z = 0;
short temp_x, temp_y, temp_z = 0;
int offsetX, offsetY, offsetZ = 0;
static ssize_t bma2x2_get_cali_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct file *fp=NULL;
	mm_segment_t old_fs;
	...
	//set calibration target 00 -> 0g, 01 -> 1g, 10 -> -1g, 11 -> 0g


	//In case sensor placement is unknown, "offset_target" is set according to Z axis output




	// start sensor fast calibration 01 -> x asix, 10 -> y axis, 11 -> z axis, 00 -> do not calibrate
	...
	{
		bma2x2_read_accel_x(bma2x2->bma2x2_client, bma2x2->sensor_type, &temp_x);
		sum_x += temp_x;
		printk("geroge sum_x=%d, temp_x=%d\n", sum_x, temp_x);
		bma2x2_read_accel_y(bma2x2->bma2x2_client, bma2x2->sensor_type, &temp_y);
		sum_y += temp_y;
		printk("geroge sum_y=%d, temp_y=%d\n", sum_y, temp_y);
		bma2x2_read_accel_z(bma2x2->bma2x2_client, bma2x2->sensor_type, &temp_z);
		sum_z  += temp_z;
		printk("geroge sum_z=%d, temp_z=%d\n",  sum_z, temp_z);
		bma2x2_delay(4);
	}
	
	offsetX = (-1) * sum_x / CAL_COUNT;
	offsetY = (-1) * sum_y / CAL_COUNT;
	if (sum_z > 0) {
		printk("sensor on top side\n");
		offsetZ = (1024 - sum_z / CAL_COUNT); // For bmc156 only
	} else if (sum_z < 0) {
		printk("sensor on bottom side\n");
		offsetZ = (-1024 - sum_z / CAL_COUNT);
	}

	sum_x = sum_y = sum_z = 0;
	temp_x = temp_y = temp_z = 0;

	printk("geroge  offsetX=%d, offsetY=%d, offsetZ=%d\n", 
		 offsetX, offsetY, offsetZ);

	fp = filp_open(CAL_PATH, O_CREAT|O_RDWR|O_SYNC, S_IRWXU|S_IRWXG|S_IRWXO);		//Open or create file
	if(IS_ERR(fp))	
	{
		printk("Create calibration file fail.\n");
	}else{
...
//	bma2x2_set_offset_x(bma2x2->bma2x2_client, cal_buf[0]); //Set offset value to sensor register 0x38
//	bma2x2_set_offset_y(bma2x2->bma2x2_client, cal_buf[1]); //Set offset value to sensor register 0x39
//	bma2x2_set_offset_z(bma2x2->bma2x2_client, cal_buf[2]); //Set offset value to sensor register 0x3A

	printk("geroge  1 cal_buf0=%d, cal_buf1=%d, cal_buf2=%d\n", 
		 cal_buf[0], cal_buf[1], cal_buf[2]);
	
	old_fs=get_fs();
	set_fs(KERNEL_DS);
	pos = 0;		//Write position start from 0
	vfs_write(fp, (u8*)cal_buf, 12, &pos);
	pos = 0;
	vfs_read(fp, cal_buf1, 12, &pos);

	printk("geroge	2 cal_buf0=%d, cal_buf1=%d, cal_buf2=%d\n", 
		(s8)cal_buf1[0], cal_buf1[1], cal_buf1[2]);
	filp_close(fp, NULL);
	set_fs(old_fs);
...
int bma2x2_update_offset(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	printk("geroge bma2x2_update_offset 1 \n");

	fp = filp_open(CAL_PATH, O_RDWR|O_SYNC, 0);		//Open file
	printk("geroge bma2x2_update_offset 2 \n");
	if(IS_ERR(fp))	
	{
		printk("Fail to open file or file not exist. \n");
		return -1;
	}
	printk("geroge bma2x2_update_offset 3 \n");
	old_fs=get_fs();
	set_fs(KERNEL_DS);
	pos = 0;
	vfs_read(fp, cal_buf, 12, &pos);
	filp_close(fp, NULL);
	set_fs(old_fs);

	printk("geroge read from calibration file offsetX=%d, offsetY=%d, offsetZ=%d\n", 
		 cal_buf[0], cal_buf[1], cal_buf[2]);

	offsetX = (s8)cal_buf[0];
	offsetY = (s8)cal_buf[1];
	offsetZ = (s8)cal_buf[2];
#endif

	switch ( offset_index ) {
	case 0:
		offsetX = data;
		printk("Geroge offsetX updated value = %d\n", offsetX);
		offset_index ++;
		break;
	case 1:
		offsetY = data;
		printk("Geroge offsetY updated value = %d\n", offsetY);
		offset_index ++;
		break;
	case 2:
		offsetZ = data;
		printk("Geroge offsetZ updated value = %d\n", offsetZ);
		offset_index = 0;
		break;
	default:
		break;
...
static ssize_t bma2x2_get_cali_offset_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int count;
	count = snprintf(buf, 96, "%d %d %d\n",
			...
			offsetY,
			offsetZ);
	PDEBUG("geroge Offset value:%d %d %d\n",
			offsetX,
			offsetY,
			offsetZ);
}
...
static DEVICE_ATTR(chipinfo, S_IRUGO,bma2x2_chipinfo_show, NULL);
...
static DEVICE_ATTR(cali, 0777,bma2x2_get_cali_show, bma2x2_update_offset);
static DEVICE_ATTR(cali_offset, 0777,bma2x2_get_cali_offset_show, NULL);
...
printk("geroge ----bma2x2_probe-- start  . \n");
...
printk("geroge ----bma2x2_probe-- end  . \n");
