...
//static const char ps_cal_file[]="/productinfo/PS_CAL.dat";
#define	ps_cal_file	"/productinfo/PS_CAL.bin"

//als calibration file location
//static const char als_cal_file[]="/data/data/com.eminent.ps.calibration/als.dat";
static const char als_cal_file[]="/productinfo/als.dat";
#if defined(CONFIG_PROJS_V2804)
static int PS_h_offset = 1400;
static int PS_l_offset = 850;
#else
...
static void epl_sensor_report_ps_status(void)
{
        ...
        LOG_INFO("----before--------------- epl_sensor.ps.data.data=%d, value=%d ir_flag_base=%x \n\n", epl_sensor.ps.data.data, epl_sensor.ps.compare_low >> 3,ir_flag_base);


        if(ir_flag_base==1)
        {
                 LOG_INFO("------------------- epl_sensor.ps.data.data=%d, value=%d ir_flag_base=%x \n\n", epl_sensor.ps.data.data, epl_sensor.ps.compare_low >> 3,ir_flag_base);
        }
        else 
        {
                epl_sensor.ps.compare_low = epl_sensor.ps.compare_low |0x08;
...
int epl_sensor_read_ps(struct i2c_client *client)
{
    ...
    if(epl_sensor.ps.data.data>40000)
         ir_flag_base=1;
...
LOG_FUN();
	...
	loff_t pos;
	char buffer[100]= {0};
         LOG_INFO("[epl_sensor.ps.factory.calibration_enable]:epl_sensor.ps.factory.calibration_enable=%d  epl_sensor.ps.factory.calibrated=%d  \n",epl_sensor.ps.factory.calibration_enable, epl_sensor.ps.factory.calibrated);
	if(epl_sensor.ps.factory.calibration_enable && !epl_sensor.ps.factory.calibrated)
	{
		...
		else
		{
			...
			epl_sensor.ps.cancelation = epl_sensor.ps.factory.cancelation;
                        ...
                        cali_base=ps_hthr-PS_h_offset;
                         LOG_INFO("read calibration file:ps_cancelation=%d  ps_hthr=%d  ps_lthr=%d cali_base=%d\n",ps_cancelation, ps_hthr,ps_lthr,cali_base);
 ...
 int g_ps_hthr=0, g_ps_lthr=0;
static int epl_run_ps_calibration_dync(struct epl_sensor_priv *epl_data)
{
...
#if 0
 LOG_INFO("[%s]: ch1 = %d \n", __func__, ch1);
    if(ir_flag==0 && ch1<5000  )
        {

               ir_driver= i2c_smbus_read_byte_data(client, 0x05)   && 0xfc;
                epl_sensor.hs.ir_driver=EPL_IR_DRIVE_200;
                ir_driver=ir_driver | EPL_IR_DRIVE_200;
                epl_sensor_I2C_Write(client, 0x05, ir_driver);
                ir_flag=1;
                ch1=0;
                msleep(150);        
                goto ir_driver_start;
    }
#endif   
    ...
    else if(ch1 <= 0)
    {
       ...
       if(ch1 > 6000)
        ...
        {
            ps_hthr = ch1 + PS_h_offset-250;
            ps_lthr = ch1 + PS_l_offset-50;
        }
       #else
            ps_hthr = ch1 + PS_h_offset;
            ps_lthr = ch1 + PS_l_offset;
	...
	LOG_INFO("[%s]: ch1 = %d ps_hthr= %d ps_lthr= %d\n", __func__, ch1,ps_hthr,ps_lthr);
...
#endif
        {
          if(epl_sensor.ps.data.data >50)    //under the sunlight
                        dynk_min_ps_raw_data = epl_sensor.ps.data.data;
        }

 LOG_INFO("[%s]:restart dynk cali_base = %d, min = %d, ir_data = %d\n", __func__, cali_base, dynk_min_ps_raw_data, epl_sensor.ps.data.ir_data);

ir_flag_base=1;
if(dynk_min_ps_raw_data>40000)
{
        dynk_min_ps_raw_data=dynk_min_ps_raw_data -4*PS_h_offset;
         LOG_INFO("[%s]:restart dynk cali_base = %d, min = %d, ir_data = %d\n", __func__, cali_base, dynk_min_ps_raw_data, epl_sensor.ps.data.ir_data);
      
}

if(dynk_min_ps_raw_data<cali_base)
{
        dynk_min_ps_raw_data=cali_base;
}

#if defined(CONFIG_PROJS_V2804)

       if(dynk_min_ps_raw_data > 6000)
        {
             dynk_thd_low = dynk_min_ps_raw_data + PS_l_offset;
	    dynk_thd_high = dynk_min_ps_raw_data + PS_h_offset;
        }
       else
        {
         dynk_thd_low = dynk_min_ps_raw_data + PS_l_offset-350;
	    dynk_thd_high = dynk_min_ps_raw_data + PS_h_offset-700;
        }
       #else
        dynk_thd_low = dynk_min_ps_raw_data + PS_l_offset;
	    dynk_thd_high = dynk_min_ps_raw_data + PS_h_offset;
...
{
        dynk_min_ps_raw_data=dynk_min_ps_raw_data -4*PS_h_offset;
         LOG_INFO("[%s]:restart dynk  min = %d\n", __func__, dynk_min_ps_raw_data);
...
static ssize_t show_chipInfo(struct device *dev,
...
static DEVICE_ATTR(chipinfo,     				S_IRUSR | S_IWUSR, 		show_chipInfo, 		NULL);
...
static int epl_sensor_ps_open(struct inode *inode, struct file *file)
{
    ...
    epld->ps_opened = 1;
    ir_flag_base=0;
  // epl_run_ps_calibration_dync(epld);
...
static void epl_sensor_early_suspend(struct early_suspend *h)
{
    ...
    if(epld->enable_pflag == 1)
    {
        //epld->ps_suspend=0;
        LOG_INFO("[%s]: ps enable \r\n", __func__);
         LOG_INFO("[%s]: ps enable ir_flag_base=%x \r\n", __func__, ir_flag_base);
        ir_flag_base=1;
    }
    else
    {
        //epld->ps_suspend=1;
        LOG_INFO("[%s]: ps disable \r\n", __func__);
         LOG_INFO("[%s]: ps disable  ir_flag_base=%x \r\n", __func__, ir_flag_base);
        epl_sensor_update_mode(epld->client);
        ir_flag_base=0;
    }
...
static int epl_sensor_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    LOG_INFO("elan sensor probe enter.\n");
		...
		LOG_INFO("elan sensor probe read dts.pdata->irq_gpio_number = %d \n",pdata->irq_gpio_number);
