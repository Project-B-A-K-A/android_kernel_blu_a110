...
static ssize_t bmm_show_chip_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	if (i != -1) {
		...
		if (op_mode != client_data->op_mode) {
			if (BMM_VAL_NAME(FORCED_MODE) == op_mode) {
				...
				if (BMM_VAL_NAME(SUSPEND_MODE) == client_data->op_mode)
				{
					
				 	/* power on firstly */
					BMM_CALL_API(set_powermode)(BMM050_ON);
				} else if (BMM_VAL_NAME(NORMAL_MODE) == client_data->op_mode)
				{
...
static DEVICE_ATTR(chip_info, S_IRUGO,bmm_show_chip_info, NULL);
